class ArchmageUpdateHandler {
   ...
   */
   prepareMigrateActorData(actor) {
    ...
    // Append NC migration for version 1.36.0
    if (this.versionBelow('1.36.0')) {
  ...
  __migrateNPCSplitSizeStrength(actor, updateData={}) {
    ...
    const sizes = Object.keys(CONFIG.HOLYGRAILWAR.creatureSizes);
    const strengths = Object.keys(CONFIG.HOLYGRAILWAR.creatureStrengths);
  ...
  async executeMigration() {
    ...
    if (this.versionBelow('1.36.0')) {
      ui.notifications.info(game.i18n.localize('ARCHMAGE.MIGRATIONS.1_36_0'), {permanent: true});
    }
