...
export class ActorArchmage extends Actor {
  ...
  applyActiveEffects(weight = 'default') {
    ...
    switch (weight) {
      ...
      case 'pre':
        relevant = (c => {return c.key.match(/system\.(abilities\..*\.value|attributes\..*\.base|attributes.recoveries.dice|attributes\.hp\.extra)/g);});
        break;
  ...
  _prepareCharacterData(data, model, flags) {
    ...
    if (data.attributes.xp) {
      ...
      data.attributes.xp.max = _lvl < 8 ? _lvl : _lvl * 2;
    }
