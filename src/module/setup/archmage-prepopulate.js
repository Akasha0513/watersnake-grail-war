...
export class ArchmagePrepopulate {
  ...
  async getCompendiums(classes = [], race = '') {
    ...
    for (let i = 0; i < classPacks.length; i++) {
      ...
      content[className] = {
        name: CONFIG.HOLYGRAILWAR.classList[className],
  ...
  async renderPowerPage(classData) {
    let template = `systems/watersnake-grail-war/templates/prepopulate/powers--list.html`;
    let templateData = {
  ...
  async renderDialog(classes = [], race = '', actor = null) {
    let validClasses = Object.keys(CONFIG.HOLYGRAILWAR.classList);
    ...
    let template = `systems/watersnake-grail-war/templates/prepopulate/tabs-content.html`;
