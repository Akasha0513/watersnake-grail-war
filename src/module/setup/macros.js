export class ArchmageMacros {
  ...
  static async aasimarHalo(speaker, actor, token, character, archmage) {
    ...
    if (aes.length > 0) {
      ...
      game.holygrailwar.MacroUtils.setDuration(effectData, CONFIG.HOLYGRAILWAR.effectDurationTypes.EndOfCombat);
  ...
  static async barbarianWhirlwind(speaker, actor, token, character, archmage) {
    ...
    game.holygrailwar.MacroUtils.setDuration(effectData, CONFIG.HOLYGRAILWAR.effectDurationTypes.StartOfNextTurn);
    await actor.createEmbeddedDocuments("ActiveEffect", [effectData]);
  }
  ...
  static async bardSongOfHeroes(speaker, actor, token, character, archmage) {
    ...
    if (targets.length == 0) targets = game.holygrailwar.MacroUtils.getAllies();
    ...
    game.holygrailwar.MacroUtils.setDuration(
      effectData,
      CONFIG.HOLYGRAILWAR.effectDurationTypes.StartOfNextSourceTurn,
      {sourceTurnUuid: actor.uuid}
    );

    // Apply effect to all targets - make the GM do it to bypass permissions
    game.holygrailwar.MacroUtils.applyActiveEffectsToTokens(targets, [effectData]);
  }
  ...
  static async bardSongOfHeroes2e(speaker, actor, token, character, archmage) {
    ...
    let bonus = game.holygrailwar.MacroUtils.getFeatsByTier(archmage.item, 'champion')[0].isActive.value ? 2 : 1;
  ...
  static async commanderOutmaneuver(speaker, actor, token, character, archmage) {
    const hasFeat = game.holygrailwar.MacroUtils.getFeatsByTier(archmage.item, 'champion')[0].isActive.value;
  ...
  static async paladinGreatDragonIncarnation(speaker, actor, token, character, archmage) {
    const effectData = {
      ...
      changes: [{
        key: "system.attributes.weapon.melee.dice",
        value: game.holygrailwar.MacroUtils.scaleDiceUp(actor.system.attributes.weapon.melee.dice),
        mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE
      }, {
        key: "system.attributes.recoveries.dice",
        value: game.holygrailwar.MacroUtils.scaleDiceUp(actor.system.attributes.recoveries.dice),
        mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE
      }]
    };
    game.holygrailwar.MacroUtils.setDuration(effectData, CONFIG.HOLYGRAILWAR.effectDurationTypes.EndOfCombat);
