      ...
      <ul class="list list--features flexcol">
        <li v-for="item in featuresByCategory(cat.key)" :key="item._id"
          ...
          <div class="feature-header flexrow">
            ...
            <a class="feature-chat-private feature-chat-icon" :data-item-id="item._id" title="선언 배너로 전송"><i class="fas fa-user-secret"></i></a>
  ...
  data() {
    return {
      ...
      categories: [
        ...
        { key: 'equipInit', label: '초기 예장', types: ['master', 'character', 'npc'], group: 'inventory' },
        { key: 'equip', label: '예장', types: ['master', 'character', 'npc'], group: 'inventory' },
        { key: 'classSkill', label: '클래스 스킬', types: ['character'], group: 'powers' },
