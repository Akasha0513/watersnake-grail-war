  ...
  <section class="section section--sidebar flexcol filters">
    ...
    <div class="unit unit--input">
      ...
      <Multiselect
        ...
        :searchable="false"
        :create-option="false"
        :options="CONFIG.HOLYGRAILWAR.creatureTypes"
      />
    </div>
    ...
    <div class="unit unit--input">
      ...
      <Multiselect
        ...
        :searchable="false"
        :create-option="false"
        :options="CONFIG.HOLYGRAILWAR.creatureStrengths"
      />
    </div>
    ...
    <div class="unit unit--input">
      ...
      <Multiselect
        ...
        :searchable="false"
        :create-option="false"
        :options="CONFIG.HOLYGRAILWAR.creatureRoles"
      />
    </div>
    ...
    <div class="unit unit--input">
      ...
      <Multiselect
        ...
        :searchable="false"
        :create-option="false"
        :options="CONFIG.HOLYGRAILWAR.creatureSizes"
      />
    </div>
        ...
        <li v-for="(entry, entryKey) in entries" :key="entryKey" :class="`creature-summary compendium-browser-row${entryKey >= pager.lastIndex - 1 && entryKey < pager.totalRows - 1 ? ' compendium-browser-row-observe': ''} flexrow document actor`" :data-document-id="entry._id" @click="openDocument(entry.uuid)">
          ...
          <div class="flexcol creature-contents" @dragstart="startDrag($event, entry, 'Actor')" draggable="true">
            ...
            <div class="grid creature-grid">
              ...
              <div class="creature-strength" :data-tooltip="localize('ARCHMAGE.strength')">{{ CONFIG.HOLYGRAILWAR.creatureStrengths[entry?.system?.details?.strength?.value] }}</div>
              <div class="creature-role" :data-tooltip="localize('ARCHMAGE.role')">{{ creatureRole(entry) }}</div>
              <div class="creature-size" :data-tooltip="localize('ARCHMAGE.size')">{{ CONFIG.HOLYGRAILWAR.creatureSizes[entry?.system?.details?.size?.value] }}</div>
...
export default {
  ...
  methods: {
    ...
    creatureRole(entry) {
      ...
      if (roleB?.value) {
        return `${CONFIG.HOLYGRAILWAR.creatureRoles[role?.value]}/${CONFIG.HOLYGRAILWAR.creatureRoles[roleB.value]}`;
      }
      return CONFIG.HOLYGRAILWAR.creatureRoles[role?.value];
    },
    creatureType(entry) {
      const {type, typeB} = entry?.system?.details || {};
      if (typeB?.value) {
        return `${CONFIG.HOLYGRAILWAR.creatureTypes[type?.value]}/${CONFIG.HOLYGRAILWAR.creatureTypes[typeB.value]}`;
      }
      return CONFIG.HOLYGRAILWAR.creatureTypes[type?.value];
    },
  },
  computed: {
    nightmode() {
  ...
  async created() {
    ...
    getPackIndex(packIds, [
      ...
      if (game.holygrailwar.system?.moduleArt?.map?.size > 0) {
