          ...
          <div class="flexcol equipment-contents" @dragstart="startDrag($event, equipment, 'Item')" draggable="true">
            ...
            <div class="grid equipment-grid">
              <div class="equipment-tier" :data-tooltip="localize('ARCHMAGE.CHAT.tier')">
                {{ CONFIG.HOLYGRAILWAR.featTiers[equipment.system.tier] }}
              </div>
              ...
              <div class="equipment-usage" v-if="equipment.system?.powerUsage?.value" :data-tooltip="localize('ARCHMAGE.GROUPS.powerUsage')">{{ CONFIG.HOLYGRAILWAR.powerUsages[equipment.system?.powerUsage?.value ?? ''] ?? '' }}</div>
...
export default {
  ...
  computed: {
    ...
    chakraSlots() {
      const result = {};
      for (let [k,v] of Object.entries(CONFIG.HOLYGRAILWAR.chakraSlots)) {
